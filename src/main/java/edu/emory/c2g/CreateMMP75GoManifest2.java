...
public class CreateMMP75GoManifest2 {  
    ...
    // args[3] = Archer SSH port
    // args[4] = Archer public URL for read stats
    // args[5] = suffix for GO run (e.g., M_batch1)
    public static void main(String[] args) throws ParseException, IOException, ClassNotFoundException, SQLException, InterruptedException {  
        ...
        System.out.println(String.format("%s\t%s\t%s\t%s", illuminaRunName + "_" + args[5] + "_" + timestamp, platform, platform + "-Myeloid-Archer6", goMount + "/" + illuminaRunName));
        ...
        while(((inLine = stdIn.readLine()) != null)) {
            ...
            for(int columnNumber = 0; columnNumber < columnNames.size(); columnNumber++) {
                ...
                switch(columnNames.get(columnNumber)) {
                    case "run_id": System.out.print(illuminaRunName + "_" + args[5] + "_" + timestamp); break;
